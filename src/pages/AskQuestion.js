...
const AskQuestion = () => {
  ...
  const decriptionDom = useRef();

  const handlePostQuestion = async () => {
    const titleValue = titleDom.current.value;
    const decriptionValue = decriptionDom.current.value;

    if (!titleValue || !decriptionValue) {
    ...
    <LayOut>
      <section className={classes.ask}>
        <div className={classes.ask_container}>
          ...
          <button onClick={handlePostQuestion}>Post Your Question</button>
        </div>
      </section>
    </LayOut>
  );
